...
def push_protected_regions_to_minecraft(server_id: str | None = None) -> tuple[bool, str]:
    """Ask connected MCC-Bridge to pull protected-region outlines via WebSocket."""
    ...
    url = (
        ...
        "/internal/minecraft/push-regions/"
    )
    request_id = f"admin-regions-{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        logger.error("[minecraft_ws] regions push request failed: %s", exc)
        return False, f"Daphne-Push fehlgeschlagen: {exc}"

    if response.status_code == 200:
        logger.info(
            "[minecraft_ws] requested regions sync push server_id=%s request_id=%s",
            target_server_id,
            request_id,
        )
        return True, f"Regionen-Sync an '{target_server_id}' gesendet"
